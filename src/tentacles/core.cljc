...
(defn parse-json
  ...
  [s] (when s #?(:clj (json/parse-string s true)
                 :cljs (js->clj (js/JSON.parse s) :keywordize-keys true))))

(defn unparse-json
  [obj]
  "Generates a JSON encoded string representing obj"
...
(defn extract-useful-meta
  ...
  (let [{:strs [etag last-modified x-ratelimit-limit x-ratelimit-remaining
    ...
    {:etag           etag :last-modified last-modified
     :call-limit     (when x-ratelimit-limit (parse-long x-ratelimit-limit))
     :call-remaining (when x-ratelimit-remaining (parse-long x-ratelimit-remaining))
     :poll-interval  (when x-poll-interval (parse-long x-poll-interval))}))
